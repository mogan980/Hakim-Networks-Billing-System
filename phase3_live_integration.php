...
"hotspot_manager.php" => [
    ...
    "api" => "hotspot_active",
    "columns" => ["User","IP Address","MAC","Uptime","Bytes In","Bytes Out"]
],
"queue_control.php" => [
    "title" => "Queue Control",
    "api" => "queues",
    "columns" => ["Name","Target","Max Limit","Disabled","Bytes"]
],
"network_health_center.php" => [
    "title" => "Network Health Center",
    "api" => "health",
...
"pppoe_manager.php" => [
    "title" => "PPPoE Manager",
    "api" => "pppoe_active",
    "columns" => ["User","IP Address","Caller ID","Uptime","Service"]
],
"traffic_analytics.php" => [
    "title" => "Traffic Analytics",
    "api" => "traffic",
    "columns" => ["Router","Download RX","Upload TX","Updated"]
],
"topology_map.php" => [
    "title" => "Topology Map",
    "api" => "topology",
    "columns" => ["Device","IP Address","MAC","Type","Status"]
]
];

$template = function($title,$api,$columns){
$ths = "";
foreach($columns as $c){ $ths .= "<th>$c</th>"; }
...
async function loadModule(){
    ...
    table.innerHTML = d.html || "<tr>$ths</tr><tr><td colspan='10'>No data</td></tr>";
}
loadModule();
...
foreach($pages as $file=>$data){
    file_put_contents(__DIR__ . "/" . $file, $template($data["title"],$data["api"],$data["columns"]));
    echo "Updated $file\n";
}

echo "PHASE 3 MODULE PAGES READY ✅\n";
