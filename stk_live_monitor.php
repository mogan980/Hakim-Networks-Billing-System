...
<style id="STK_CLEAN_UI">
.stk-status-paid{background:#dcfce7;color:#166534;padding:6px 10px;border-radius:999px;font-weight:900;font-size:11px}
.stk-status-pending{background:#fef3c7;color:#92400e;padding:6px 10px;border-radius:999px;font-weight:900;font-size:11px}
.stk-status-failed{background:#fee2e2;color:#991b1b;padding:6px 10px;border-radius:999px;font-weight:900;font-size:11px}
.stk-live-time{color:#94a3b8;font-size:12px;margin-top:8px}
table tr:hover{background:rgba(34,197,94,.06)}
</style>
\n
<style id="STK_REMOVE_DUPLICATE_VALUES">
/* Hide old duplicate small values inside STK summary cards */
.stk-card small,
.stat-card small,
.card small{
...
/* Make the live main value the only visible value */
.stk-card h2,
.stk-card h3,
.stat-card h2,
.stat-card h3,
...
\n
<script id="HN_STK_MONITOR_LIVE_CLEAN">
document.addEventListener("DOMContentLoaded",()=>{

  let stkLimit = localStorage.getItem("stkLimit") || "10";
  ...
  function badge(status){
    ...
    if(status==="paid") return '<span class="stk-status-paid">PAID</span>';
    if(status==="pending") return '<span class="stk-status-pending">PENDING</span>';
    return '<span class="stk-status-failed">'+status.toUpperCase()+'</span>';
  }

  function ensureControls(){
    const search = document.querySelector('input[placeholder*="Search"]');
    if(!search || document.getElementById("stkLimitSelect")) return;

    const select=document.createElement("select");
    select.id="stkLimitSelect";
    select.innerHTML=`
      <option value="10">Show 10</option>
      <option value="20">Show 20</option>
      <option value="all">Show All</option>
    `;
    select.value=stkLimit;
    select.style.cssText="background:#020617;color:white;border:1px solid #334155;border-radius:12px;padding:11px 14px;font-weight:800;margin-left:10px";
    select.onchange=()=>{
      stkLimit=select.value;
      localStorage.setItem("stkLimit",stkLimit);
      syncSTK();
    };
  ...
  function setBigPanelAmount(value){
    const panel=[...document.querySelectorAll("div")].find(x=>{
      const t=(x.innerText||"").trim();
      return t.startsWith("KES") && x.querySelector("input");
    });
    if(panel){
      const first=[...panel.querySelectorAll("h1,h2,h3,b,strong,div")].find(x=>(x.textContent||"").trim().startsWith("KES"));
      if(first) first.textContent=value;
    }
  }

  async function syncSTK(){
    try{
      ensureControls();

      const r=await fetch("stk_monitor_live_api.php?limit="+encodeURIComponent(stkLimit)+"&_="+Date.now(),{cache:"no-store"});
      const d=await r.json();
      if(!d.success) return;

      setBigPanelAmount(money(d.total_revenue));

      document.querySelectorAll(".card,.stat-card").forEach(card=>{
        const t=card.innerText||"";
        const num=card.querySelector("h2,h3,b,strong");
        if(!num) return;

        if(t.includes("Total Revenue")) num.textContent=money(d.total_revenue);
        if(t.includes("Today")) num.textContent=money(d.today_revenue);
        if(t.includes("Paid Sessions")) num.textContent=d.paid;
        if(t.includes("Pending / Failed")) num.textContent=d.pending+" / "+d.failed;
      });

      const table=[...document.querySelectorAll("table")].find(t=>{
        const x=t.innerText||"";
        return x.includes("Phone") && x.includes("Client IP");
      });
        ...
        if(tbody){
          tbody.innerHTML=d.rows.map(row=>`
            <tr>
              <td>${row.id||""}</td>
              <td>${row.phone||"-"}</td>
              <td>KES ${Number(row.amount||0).toLocaleString()}</td>
              <td>${badge(row.status)}</td>
              <td>${row.mpesa_receipt || row.reference || "-"}</td>
              <td style="max-width:250px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${row.checkout_id || "-"}</td>
              <td>${row.client_ip || "-"}</td>
      ...
      let live=document.getElementById("stkLiveTime");
      if(!live){
        live=document.createElement("div");
        live.id="stkLiveTime";
        live.style.cssText="color:#94a3b8;font-size:12px;margin:8px 0";
        const h=[...document.querySelectorAll("h1,h2")].find(x=>(x.textContent||"").includes("Recent STK"));
        if(h) h.insertAdjacentElement("afterend",live);
      }
      live.textContent="Live updated: "+d.time+" • showing "+stkLimit;

    }catch(e){}
  }

  syncSTK();
  setInterval(syncSTK,10000);
});
</script>
\n
<script id="HN_STK_CARD_REBUILD">
document.addEventListener("DOMContentLoaded",()=>{
  ...
  async function rebuildSTKCards(){
    try{
      const r = await fetch("stk_monitor_live_api.php?limit=10&_="+Date.now(), {cache:"no-store"});
      const d = await r.json();
      if(!d.success) return;

      const cards = [...document.querySelectorAll(".card,.stat-card")].slice(0,4);

      const data = [
        ["Total Revenue", money(d.total_revenue)],
        ["Today", money(d.today_revenue)],
        ["Paid Sessions", d.paid],
        ["Pending / Failed", d.pending + " / " + d.failed]
      ];
      ...
      cards.forEach((card,i)=>{
        ...
        card.innerHTML = `
          <div style="font-size:14px;color:#94a3b8;font-weight:800;margin-bottom:14px">
            ${data[i][0]}
          </div>
          <div style="font-size:30px;color:#22c55e;font-weight:900">
            ${data[i][1]}
          </div>
        `;
      });
  ...
  rebuildSTKCards();
  setInterval(rebuildSTKCards,10000);
});
</script>
