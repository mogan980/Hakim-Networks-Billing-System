...
$update->execute([
    ...
    // VOUCHER_REVENUE_INSERT_SAFE
    try {
        $packageId = $voucher["package_id"] ?? null;
        $amount = 0;

        if ($packageId) {
            $pkg = $pdo->prepare("SELECT price FROM packages WHERE id=? LIMIT 1");
            $pkg->execute([$packageId]);
            $amount = (float)($pkg->fetchColumn() ?: 0);
        }

        if ($amount <= 0) {
            $amount = (float)($voucher["price"] ?? $voucher["amount"] ?? 0);
        }

        $exists = $pdo->prepare("SELECT id FROM payments WHERE reference=? OR checkout_id=? LIMIT 1");
        $exists->execute([$code, "VOUCHER-" . $code]);
        ...
        if (!$exists->fetch()) {
            $pay = $pdo->prepare("
                ...
                VALUES (?, ?, 'voucher', ?, 'paid', ?, ?, ?, NOW())
            ");
            $pay->execute([$packageId, $amount, $code, "VOUCHER-" . $code, $code, $clientIp]);
        }
    } catch(Exception $e) {
        error_log("Voucher revenue insert failed: " . $e->getMessage());
    }

    // Record voucher redemption as paid revenue
    try {
        $packageId = $voucher["package_id"] ?? null;
        $amount = 0;
        ...
        if ($packageId) {
            ...
            $amount = (float)($pkgStmt->fetchColumn() ?: 0);
        }
        ...
        error_log("Voucher payment insert failed: " . $e->getMessage());
    }
