...
try{
    ...
    if($module === "hotspot_active"){
        ...
        $html = "<tr><th>User</th><th>IP Address</th><th>MAC</th><th>Uptime</th><th>Bytes In</th><th>Bytes Out</th></tr>";
        foreach($rows as $r){
            $html .= "<tr>".td($r["user"] ?? "-").td($r["address"] ?? "-").td($r["mac-address"] ?? "-").td($r["uptime"] ?? "-").td($r["bytes-in"] ?? "0").td($r["bytes-out"] ?? "0")."</tr>";
        }
        $count = count($rows);
    }
    ...
    elseif($module === "queues"){
        ...
        $html = "<tr><th>Name</th><th>Target</th><th>Max Limit</th><th>Disabled</th><th>Bytes</th></tr>";
        foreach($rows as $r){
            $html .= "<tr>".td($r["name"] ?? "-").td($r["target"] ?? "-").td($r["max-limit"] ?? "-").td($r["disabled"] ?? "-").td($r["bytes"] ?? "-")."</tr>";
        }
        $count = count($rows);
    }
    ...
    elseif($module === "pppoe_active"){
        ...
        $html = "<tr><th>User</th><th>IP Address</th><th>Caller ID</th><th>Uptime</th><th>Service</th></tr>";
        foreach($rows as $r){
            $html .= "<tr>".td($r["name"] ?? "-").td($r["address"] ?? "-").td($r["caller-id"] ?? "-").td($r["uptime"] ?? "-").td($r["service"] ?? "-")."</tr>";
        }
        $count = count($rows);
    }
