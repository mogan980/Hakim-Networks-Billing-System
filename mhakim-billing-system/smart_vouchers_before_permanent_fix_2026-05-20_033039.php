...
if(isset($_POST["generate"])){
    ...
    for($i=0; $i<$qty; $i++){
        do {
            $code = makeCode();
            $check = $pdo->prepare("SELECT id FROM smart_vouchers WHERE voucher_code=? LIMIT 1");
            $check->execute([$code]);
        } while($check->fetch());
...
$vouchers = $pdo->query("SELECT * FROM smart_vouchers ORDER BY id DESC LIMIT 300")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html>
<head>
...
input,select{padding:13px;border:1px solid #cbd5e1;border-radius:12px;width:100%;box-sizing:border-box}
...
<p>Generate secure hotspot vouchers with package speed, duration and price.</p>

<div class="card">
<h2>Generate Vouchers</h2>
<form method="POST">
<div class="grid">
<input name="package_name" placeholder="Package Name e.g. 24 Hours" value="24 Hours" required>
...
<input name="qty" type="number" placeholder="Quantity" value="10" required>
</div>
<br>
...
<td><?php echo htmlspecialchars($v["duration_hours"]); ?> hrs</td>
